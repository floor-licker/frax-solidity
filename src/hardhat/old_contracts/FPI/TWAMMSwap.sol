...
contract TWAMMSwap is Ownable {
    ...
    function setTWAMMTrade(uint _buy0PerSecond,uint _buy0EndTime, uint _buy1PerSecond, uint _buy1EndTime) external lock onlyOwner {
        ...
        uint balance0 = IERC20(token0).balanceOf(address(this)) + sold0 - bought0;
        uint balance1 = IERC20(token1).balanceOf(address(this)) + sold1 - bought1;
    ...
    function removeLiquidity(uint percentage) external lock onlyOwner returns (uint amount0, uint amount1) {
        ...
        balance0 = IERC20(token0).balanceOf(address(this)) + sold0 - bought0;
        balance1 = IERC20(token1).balanceOf(address(this)) + sold1 - bought1;
        _update(balance0, balance1);
    }
    ...
    function skim(address to) external lock {
        ...
        SafeERC20.safeTransfer(_token0, to, _token0.balanceOf(address(this)) + bought0 - sold0 - reserve0);
        SafeERC20.safeTransfer(_token1, to, _token1.balanceOf(address(this)) + bought1 - sold1 - reserve1);
    }

    // force reserves to match balances
    function sync() external lock {
    	_executeTWAMMTrade();
        _update(IERC20(token0).balanceOf(address(this)) + sold0 - bought0, IERC20(token1).balanceOf(address(this)) + sold1 - bought1);
    }
    ...
    function swap(uint amount0Out, uint amount1Out, address to) internal {
        ...
        balance0 = IERC20(_token0).balanceOf(address(this)) + sold0 - bought0;
        balance1 = IERC20(_token1).balanceOf(address(this)) + sold1 - bought1;
        }
        ...
        require(balance0Adjusted * balance1Adjusted >= uint(_reserve0) * _reserve1 * 1000**2, 'TWAPP: K');
        }
        _update(balance0, balance1);
