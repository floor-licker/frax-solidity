...
contract CurveAMO_V2 is AccessControl {
    ...
    function showAllocations() public view returns (uint256[12] memory return_arr) {
        ...
        // Free LP
        uint256 lp_owned = (frax3crv_metapool.balanceOf(address(this))) + lp_lent_to_voter;

        // Staked in the gauge
        uint256 lp_in_gauge = gauge_frax3crv.balanceOf(address(this));
        lp_owned = lp_owned + lp_in_gauge;
        ...
        return [
            frax_in_contract, // [0]
            frax_withdrawable, // [1]
            frax_withdrawable + frax_in_contract, // [2]
            usdc_in_contract, // [3]
            usdc_withdrawable, // [4]
            usdc_subtotal, // [5]
            usdc_subtotal + (frax_in_contract + frax_withdrawable) * fraxDiscountRate() / 1e6 * (10 ** missing_decimals), // [6] USDC Total
            lp_owned, // [7]
            frax3crv_supply, // [8]
    ...
    //     return integrate_fraction - other_side;
    // }
