...
contract StakingRewardsDualV3 is Owned, ReentrancyGuard {
    ...
    function veFXSMultiplier(address account) public view returns (uint256) {
        ...
        if (veFXS_needed_for_max_boost > 0){ 
            ...
            uint256 vefxs_multiplier = uint256(MULTIPLIER_PRECISION).add(
                (user_vefxs_fraction)
                     * vefxs_max_multiplier - MULTIPLIER_PRECISION
        ...
        )
    {
        ...
        for (uint256 i = 0; i < lockedStakes[account].length; i++) {
            ...
            uint256 extra_vefxs_boost = midpoint_vefxs_multiplier - MULTIPLIER_PRECISION; // Multiplier - 1, because it is additive
            uint256 combined_boosted_amount = liquidity * lock_multiplier + extra_vefxs_boost / MULTIPLIER_PRECISION;
