...
contract FraxPool is AccessControl, Owned {
    ...
    function collatDollarBalance() public view returns (uint256) {
        if(collateralPricePaused == true){
            ...
            return (collateral_token.balanceOf(address(this)) - unclaimedPoolCollateral) * 10 ** missing_decimals * collat_usd_price / PRICE_PRECISION; // * getCollateralPrice() / 1e6;    
        }
    }
    ...
    function availableExcessCollatDV() public view returns (uint256) {
        ...
        uint256 required_collat_dollar_value_d18 = (total_supply * global_collateral_ratio) / COLLATERAL_RATIO_PRECISION; // Calculates collateral needed to back each 1 FRAX with $1 of collateral at current collat ratio
        if (global_collat_value > required_collat_dollar_value_d18) return global_collat_value - required_collat_dollar_value_d18;
        else return 0;
    }
    ...
    function getCollateralPrice() public view returns (uint256) {
        if(collateralPricePaused == true){
            ...
            uint256 eth_usd_price = FRAX.eth_usd_price();
            return eth_usd_price * PRICE_PRECISION / collatEthOracle.consult(weth_address, PRICE_PRECISION * (10 ** missing_decimals));
        }
    }
    ...
    function mint1t1FRAX(uint256 collateral_amount, uint256 FRAX_out_min) external notMintPaused {
        ...
        frax_amount_d18 = (frax_amount_d18 * uint(1e6 - minting_fee)) / 1e6; //remove precision at the end
    ...
    function mintAlgorithmicFRAX(uint256 fxs_amount_d18, uint256 FRAX_out_min) external notMintPaused {
        ...
        frax_amount_d18 = (frax_amount_d18 * uint(1e6 - minting_fee)) / 1e6;
        require(FRAX_out_min <= frax_amount_d18, "Slippage limit reached");
    ...
    function mintFractionalFRAX(uint256 collateral_amount, uint256 fxs_amount, uint256 FRAX_out_min) external notMintPaused {
        ...
        mint_amount = (mint_amount * uint(1e6 - minting_fee)) / 1e6;
    ...
    function redeem1t1FRAX(uint256 FRAX_amount, uint256 COLLATERAL_out_min) external notRedeemPaused {
        ...
        uint256 FRAX_amount_precision = FRAX_amount / 10 ** missing_decimals;
        (uint256 collateral_needed) = FraxPoolLibrary.calcRedeem1t1FRAX(
            getCollateralPrice(),
            FRAX_amount_precision
        );

        collateral_needed = (collateral_needed * uint(1e6 - redemption_fee)) / 1e6;
    ...
    function redeemFractionalFRAX(uint256 FRAX_amount, uint256 FXS_out_min, uint256 COLLATERAL_out_min) external notRedeemPaused {
        ...
        uint256 FRAX_amount_post_fee = (FRAX_amount * uint(1e6 - redemption_fee)) / PRICE_PRECISION;

        uint256 fxs_dollar_value_d18 = FRAX_amount_post_fee - FRAX_amount_post_fee * global_collateral_ratio / PRICE_PRECISION;
        uint256 fxs_amount = fxs_dollar_value_d18 * PRICE_PRECISION / fxs_price;

        // Need to adjust for decimals of collateral
        uint256 FRAX_amount_precision = FRAX_amount_post_fee / 10 ** missing_decimals;
        uint256 collateral_dollar_value = FRAX_amount_precision * global_collateral_ratio / PRICE_PRECISION;
        uint256 collateral_amount = collateral_dollar_value * PRICE_PRECISION / col_price_usd;


        require(collateral_amount <= collateral_token.balanceOf(address(this)) - unclaimedPoolCollateral, "Not enough collateral in pool");
        ...
        redeemFXSBalances[msg.sender] = redeemFXSBalances[msg.sender] + fxs_amount;
    ...
    function redeemAlgorithmicFRAX(uint256 FRAX_amount, uint256 FXS_out_min) external notRedeemPaused {
        ...
        fxs_dollar_value_d18 = (fxs_dollar_value_d18 * uint(1e6 - redemption_fee)) / PRICE_PRECISION; //apply fees
    ...
    function recollateralizeFRAX(uint256 collateral_amount, uint256 FXS_out_min) external {
        ...
        uint256 fxs_paid_back = amount_to_recollat * uint(1e6 + bonus_rate - recollat_fee) / fxs_price;
    ...
    function buyBackFXS(uint256 FXS_amount, uint256 COLLATERAL_out_min) external {
        ...
        (uint256 collateral_equivalent_d18) = (FraxPoolLibrary.calcBuyBackFXS(input_params)) * uint(1e6 - buyback_fee) / 1e6;
