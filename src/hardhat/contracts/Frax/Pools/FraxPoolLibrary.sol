...
library FraxPoolLibrary {
    ...
    function calcMintAlgorithmicFRAX(uint256 fxs_price_usd, uint256 fxs_amount_d18) public pure returns (uint256) {
        return fxs_amount_d18 * fxs_price_usd / 1e6;
    }
    ...
    function calcMintFractionalFRAX(MintFF_Params memory params) internal pure returns (uint256, uint256) {
        ...
        {    
            ...
            fxs_dollar_value_d18 = params.fxs_amount * params.fxs_price_usd / 1e6;
        ...
        uint calculated_fxs_needed = calculated_fxs_dollar_value_d18 * 1e6 / params.fxs_price_usd;

        return (
            c_dollar_value_d18 + calculated_fxs_dollar_value_d18,
            calculated_fxs_needed
        );
    }

    function calcRedeem1t1FRAX(uint256 col_price_usd, uint256 FRAX_amount) public pure returns (uint256) {
        return FRAX_amount * 1e6 / col_price_usd;
    }
    ...
    function calcBuyBackFXS(BuybackFXS_Params memory params) internal pure returns (uint256) {
        ...
        uint256 fxs_dollar_value_d18 = params.FXS_amount * params.fxs_price_usd / 1e6;
        ...
        uint256 collateral_equivalent_d18 = fxs_dollar_value_d18 * 1e6 / params.col_price_usd;
    ...
    function calcRecollateralizeFRAXInner(
        ...
        uint256 effective_collateral_ratio = global_collat_value * 1e6 / frax_total_supply; //returns it in 1e6
        uint256 recollat_possible = (global_collateral_ratio * frax_total_supply - frax_total_supply * effective_collateral_ratio) / 1e6;
